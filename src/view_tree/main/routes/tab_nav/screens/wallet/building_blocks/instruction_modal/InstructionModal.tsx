...
import {
    backToFeedContent,
    explainDigicoinUsesContent,
    explainWalletContent,
    promptCollectContent,
...
const InstructionModal: React.FC<Props> = (props) => {
    ...
    switch (tutorialScreen) {
        case TutorialScreen.ExplainWallet:
            currentScreen = (
                <TutorialModal
                    top={false}
                    goBackScreen={TutorialScreen.PromptOpenWallet}
                    content={explainWalletContent}
                    goBack={props.nav2MainFeed}
                />
            );
            break;
        case TutorialScreen.PromptCollect:
            currentScreen = (
                <TutorialModal
                    top={false}
                    goBackScreen={TutorialScreen.ExplainWallet}
                    content={promptCollectContent}
                />
            );
            break;
        case TutorialScreen.ExplainDigicoinUses:
            currentScreen = (
                <TutorialModal
                    top={false}
                    goBack={props.resetCollectTrans}
                    goBackScreen={TutorialScreen.PromptCollect}
                    content={explainDigicoinUsesContent}
                />
            );
            break;
        case TutorialScreen.BackToFeed:
            currentScreen = (
                <TutorialModal
                    top
                    goBackScreen={TutorialScreen.ExplainDigicoinUses}
                    content={backToFeedContent}
                />
            );
            break;
